<script>
    ...
    function toggle() {
        ...
        if (upcoming_theme === (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light')) {
    ...
    const cb = (e) => theme.set({ preference: $theme.preference, current: e.matches ? 'dark' : 'light' });
    ...
    onMount(() => {
        (document.getElementById('container') || {}).className = $theme.current === 'system' ? '' : `highcharts-${$theme.current}`;
